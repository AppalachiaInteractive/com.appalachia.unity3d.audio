...
public class Monitor : EditorWindow {
    ...
    protected void OnSceneGUI(SceneView sv) {
        ...
        var c = UnityEditor.Handles.color;

        for (var i = Synthesizer.activeSources0.GetEnumerator(); i.MoveNext();)
            if (i.Current.keyOn <= 0f) {
                int j = GetSourceColor(i.Current.info.audioSource.name);
                UnityEditor.Handles.color = ColorizeDrawer.GetColor(j);
                var q = i.Current.info.audioSource.transform.position;
                var d = p - q;
                if (d.magnitude < 1f)
                    continue;
                var e = d.normalized;
                UnityEditor.Handles.DrawLine(p - e*0.5f, q + e*0.5f);
                ...
                var k = GUI.color;
                GUI.color = UnityEditor.Handles.color;
                var a = p - d * 0.5f;
    ...
    void DrawOcclusionGUI() {
        ...
        if (Application.isPlaying) {
            ...
            _srcInfo.Sort((x, y) => String.CompareOrdinal(x.info.audioSource.name, y.info.audioSource.name));
